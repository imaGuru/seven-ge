...
public class EntityManagerTest extends AndroidTestCase {
	...
	public void testCreateEntity() {
		...
		if (testEntity2 == null)
			Assert.fail("Null entity");
		if (testEntity2.mId != testEntity.mId)
			Assert.fail("ID mismatch");
	}

	public void testRemoveEntity() {
		Entity[] testEntities = new Entity[] { mEM.createEntity(5),
				mEM.createEntity(5), mEM.createEntity(5), mEM.createEntity(5),
				mEM.createEntity(5) };
		if (mEM.removeEntity(testEntities[2].mId) == null)
			Assert.fail("Null entity");
		if (mEM.removeEntity(testEntities[2].mId) != null)
			Assert.fail("Entity not removed");
		if (mEM.removeEntity(testEntities[1].mId) == null)
			Assert.fail("Entity should exist");
		if (mEM.removeEntity(testEntities[3].mId) == null)
			Assert.fail("Entity should exist");
	}
	...
	public void testAssignEntities() {
		...
		Entity[] testEntities = new Entity[] {
				mEM.createEntity(new Component[1], 4),
				mEM.createEntity(new Component[1], 4), mEM.createEntity(5),
				mEM.createEntity(5), mEM.createEntity(new Component[1], 5) };
		mEM.registerSystem(ts);
		mEM.assignEntities();
		if (ts.getCount() != correntNumberOfAssignedEntites)
			Assert.fail("Incorrectly assigned entites");
