			...
			if ((SYSTEM_MASK & entity.mask) == SYSTEM_MASK) {
				...
				for (InputEvent curIE : inputEvents) {
					if (curIE.type == InputEvent.Type.SCROLL) {
						...
						int x = (int)(curIE.motionEvent2.getX() + curIE.distX);
						int y = (int)(curIE.motionEvent2.getY() + curIE.distY);
						Camera2D.unProject(x, y, cc.width, cc.height, devCoords, cc.invertedVPMatrix, coords);
						float x1 = coords[0];
						float y1 = coords[1];
						x = (int)curIE.motionEvent2.getX();
						y = (int)curIE.motionEvent2.getY();
						Camera2D.unProject(x, y, cc.width, cc.height, devCoords, cc.invertedVPMatrix, coords);
						float x2 = coords[0];
						float y2 = coords[1];
