	...
	public void load(String content) {
		try {
			...
			for (int i = 0; i < jarr.length(); i++) {
				...
				JSONArray jTextureRegions = jSpriteSheetData
						.getJSONArray("textureRegions");
				for (int j = 0; j < jTextureRegions.length(); j++) {
					JSONObject jRegion = jTextureRegions.getJSONObject(j);
					assetManager.registerAsset(
							jRegion.getString("id"),
							new TextureRegion(jRegion.getInt("width"), jRegion
									.getInt("height"), jRegion.getInt("x"),
									jRegion.getInt("y"), (Texture) assetManager
											.getAsset(textureId)));
				}
			}
