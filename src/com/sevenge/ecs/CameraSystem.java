...
public class CameraSystem extends System implements GestureProcessor {
  ...
  public boolean onScroll(MotionEvent me1, MotionEvent me2, float distX, float distY) {
    ...
    CameraComponent cc = (CameraComponent) camera.components.get(8);
    Camera2D.unproject((int) (me2x + distX), (int) (me2y + distY), cc.width, cc.height, devCoords,
        cc.invertedVPMatrix, coords);
    float x1 = coords[0];
    float y1 = coords[1];
    Camera2D.unproject((int) me2x, (int) me2y, cc.width, cc.height, devCoords, cc.invertedVPMatrix,
        coords);
    float x2 = coords[0];
    float y2 = coords[1];
    PositionComponent cp = (PositionComponent) camera.components.get(1);
    cc.px = cc.x;
    cc.py = cc.y;
    cc.x -= x2 - x1;
    cc.y -= y2 - y1;
