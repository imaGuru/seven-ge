...
public class FontUtils {
	 ...
	 * @return loaded font
	 */
	public static Font load(AssetManager as, String ttfFont, int size,
			int padX, int padY) {
		...
		font.charWidths = new float[CHAR_CNT]; // Width of Each Character
												// (Actual; Pixels)
		font.charRgn = new TextureRegion[CHAR_CNT]; // Region of Each Character
													// (Texture Coordinates)
		...
		for (char c = CHAR_START; c <= CHAR_END; c++) { // FOR Each Character
			...
			if (font.charWidths[cnt] > charWidthMax) // IF Width Larger Than Max
														// Width
		...
		font.cellWidth = (int) charWidthMax + (2 * font.fontPadX); // Set Cell
																	// Width
		font.cellHeight = (int) charHeight + (2 * font.fontPadY); // Set Cell
																	// Height
		int maxSize = font.cellWidth > font.cellHeight ? font.cellWidth
				: font.cellHeight; // Save Max Size (Width/Height)
		...
		Bitmap bitmap = Bitmap.createBitmap(textureSize, textureSize,
				Bitmap.Config.ARGB_8888); // Create
											// Bitmap
		...
		float y = (font.cellHeight - 1) - fontDescent - font.fontPadY; // Set
																		// Start
																		// Position
																		// (Y)
		for (char c = CHAR_START; c <= CHAR_END; c++) { // FOR Each Character
			...
			if ((x + font.cellWidth - font.fontPadX) > textureSize) { // IF End
																		// of
																		// Line
																		// Reached
		...
		for (int c = 0; c < CHAR_CNT; c++) { // FOR Each Character (On Texture)
			font.charRgn[c] = new TextureRegion(font.cellWidth - 1,
					font.cellHeight - 1, (int) x, (int) y, tex);
	...
	public static float getLength(String text, Font font) {
		...
		len += (strLen > 1 ? ((strLen - 1) * font.spaceX) * font.scaleX : 0); // Add
																				// Space
																				// Length
