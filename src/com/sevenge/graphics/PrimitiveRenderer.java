...
public class PrimitiveRenderer {
	 ...
	 * 
	 * @param size
	 *            maximum vertices in a batch call
	 */
	public PrimitiveRenderer(int size) {
		...
		mPrimitives = new float[mSize
				* (POSITION_COMPONENT_COUNT + COLOR_COMPONENT_COUNT)];
		mVertexArray = new VertexArray(mSize
				* (POSITION_COMPONENT_COUNT + COLOR_COMPONENT_COUNT));
		mIndexBuffer = ByteBuffer.allocateDirect(mSize * 2 * BYTES_PER_SHORT)
	...
	public void begin() {
		...
		mVertexArray.setVertexAttribPointer(0,
				shader.attributePositionLocation, POSITION_COMPONENT_COUNT,
				STRIDE);
		mVertexArray.setVertexAttribPointer(POSITION_COMPONENT_COUNT,
				shader.atributeColorLocation, COLOR_COMPONENT_COUNT, STRIDE);
	}
	...
	public void flush() {
		...
		mVertexArray.put(mPrimitives, mPrimitiveCount
				* (POSITION_COMPONENT_COUNT + COLOR_COMPONENT_COUNT));
		if (mFillEnabled) {
			mIndexBuffer.put(mIndices, 0, mPrimitiveCount * 3);
			mIndexBuffer.position(0);
			glDrawElements(GL_TRIANGLES, mPrimitiveCount * 3,
					GL_UNSIGNED_SHORT, mIndexBuffer);
		} else {
			mIndexBuffer.put(mIndices, 0, mPrimitiveCount * 2);
			mIndexBuffer.position(0);
			glDrawElements(GL_LINES, mPrimitiveCount * 2, GL_UNSIGNED_SHORT,
					mIndexBuffer);
		}
		if (mVertexCountPeak < mPrimitiveCount)
			mVertexCountPeak = mPrimitiveCount;
		mPrimitiveCount = 0;
		mVertexArray.clear();
	}
	 ...
	 *            angle in radians
	 * @param nsegments
	 *            number of vertices approximating circle
	 * @param red
	 *            component of color
	 * @param green
	 *            component of color
	 ...
	 *            component of color
	 */
	public void drawCircle(float cx, float cy, float r, float rotation,
			int nsegments, float red, float green, float blue) {
		if (mPrimitiveCount == mSize)
			flush();
		float t, x = (float) Math.cos(-rotation) * r, y = (float) Math
				.sin(-rotation) * r;
		...
		for (short i = 0; i < nsegments; i++) {
			...
			mIndices[offset + i * 2 + 1] = (short) (mPrimitiveCount + (i + 1)
					% nsegments);
		}
	 ...
	 *            component of color
	 */
	public void drawLine(float x, float y, float x2, float y2, float red,
	 ...
	 *            component of color
	 */
	public void drawAngleLine(float x, float y, float angle, float length,
	 ...
	 *            component of color
	 */
	public void drawVector(float ox, float oy, float xdist, float ydist,
			float arrowWidth, float arrowHeightFactor, float red, float green,
			float blue) {
